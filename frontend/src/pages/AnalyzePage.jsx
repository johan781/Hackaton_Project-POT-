...
function DebugPanel({ analysis }) {
  ...
  if (!debug) return <div className="text-brand-gray text-sm p-4">No hay datos de debug disponibles.</div>

  return (
    <div className="space-y-4 text-xs font-mono">
      {/* File info */}
      <div className="bg-gray-900 text-green-400 rounded p-4 space-y-1">
      ...
      {debug.input_preview && (
        <div>
          <div className="text-brand-gray uppercase text-xs mb-1 font-sans">
            Contenido enviado a Claude (primeros 3000 chars)
          </div>
          <pre className="bg-gray-900 text-gray-300 rounded p-4 overflow-x-auto whitespace-pre-wrap break-all text-xs max-h-64 overflow-y-auto">
            {debug.input_preview}
          </pre>
        </div>
      )}
      ...
      <div>
        ...
        <pre className="bg-gray-900 text-blue-300 rounded p-4 overflow-x-auto whitespace-pre-wrap break-all text-xs max-h-96 overflow-y-auto">
      ...
      <div>
        ...
        <pre className="bg-gray-900 text-yellow-300 rounded p-4 overflow-x-auto whitespace-pre-wrap break-all text-xs max-h-96 overflow-y-auto">
...
export default function AnalyzePage() {
  ...
  return (
    <div className="max-w-5xl mx-auto p-6 space-y-6">
      {/* Page title */}
      <div>
        <h1 className="text-2xl font-bold text-brand-dark">Análisis de Archivo POT</h1>
      ...
      {!analysis && (
        <div
          className={`relative border-2 border-dashed rounded p-10 text-center transition-colors cursor-pointer
            ${dragging ? 'border-brand bg-brand/5' : 'border-gray-300 bg-brand-light hover:border-brand hover:bg-brand/5'}
        ...
        >
          ...
          <FileIcon className={`w-12 h-12 mx-auto mb-4 ${file ? 'text-brand' : 'text-brand-gray/40'}`} />

          {!file ? (
            <>
              <p className="text-brand-dark font-semibold text-lg">Arrastra tu archivo aquí</p>
              <p className="text-brand-gray text-sm mt-1">o haz clic para seleccionar</p>
              <p className="text-brand-gray/50 text-xs mt-3">Formatos soportados: PDF, XLSX, XLS, TXT</p>
            </>
          ) : (
            <>
              <p className="text-brand font-semibold text-lg">{file.name}</p>
              <p className="text-brand-gray text-sm mt-1">{(file.size / 1024 / 1024).toFixed(2)} MB · Listo para analizar</p>
            </>
          )}
        </div>
      )}
      ...
      {(status === 'uploading' || status === 'analyzing') && (
        <div className="space-y-2">
          ...
          <div className="w-full bg-gray-200 rounded h-2">
            <div className="bg-brand h-2 rounded transition-all duration-300"
              style={{ width: `${progress}%` }} />
          </div>
          <p className="text-xs text-brand-gray">Esto puede tomar 15-40 segundos según el tamaño del archivo.</p>
        </div>
      )}
      ...
      {!analysis && (
        <div className="flex gap-3">
          <button onClick={runAnalysis}
            ...
            className="flex items-center gap-2 bg-brand text-white px-6 py-2.5 rounded font-semibold hover:bg-brand/90 disabled:opacity-40 disabled:cursor-not-allowed transition-colors">
            {status === 'uploading' || status === 'analyzing'
          ...
          {file && status === 'idle' && (
            <button onClick={handleDiscard}
              className="flex items-center gap-2 text-brand-gray border border-gray-200 px-4 py-2.5 rounded hover:bg-brand-light transition-colors">
      ...
      {analysis && (
        <div className="space-y-4">
          {/* Action bar */}
          <div className="flex flex-wrap items-center gap-3 bg-white border border-gray-200 rounded p-4">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              {/* Discard */}
              <button onClick={handleDiscard}
                className="flex items-center gap-1.5 text-sm border border-gray-200 text-brand-gray px-3 py-2 rounded hover:bg-brand-light transition-colors">
                <Trash2 className="w-4 h-4" />
                Descartar
              </button>
              ...
              {savedId ? (
                <button onClick={() => navigate(`/projects/${savedId}`)}
                  className="flex items-center gap-1.5 text-sm bg-green-600 text-white px-4 py-2 rounded hover:bg-green-700 transition-colors">
                  <Download className="w-4 h-4" />
                  Ver proyecto guardado
                </button>
              ) : (
                <button onClick={handleSave} disabled={saving}
                  className="flex items-center gap-1.5 text-sm bg-brand text-white px-4 py-2 rounded hover:bg-brand/90 disabled:opacity-50 transition-colors">
          ...
          {activeTab === 'report' ? (
            ...
            <div className="bg-brand-light rounded border border-gray-200 p-5">
